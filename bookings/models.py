...
class TransportationRequest(models.Model):
    ...
    def __str__(self):
        return f"{self.request_number} - {self.customer.company_name} [{self.get_booking_status_display()}]"
    ...
    def billed_amount(self):
        """Returns the final customer price or paid amount, or a standard estimate if 0/missing."""
        ...
        # Authentic corporate fare estimate based on distance and vehicle category
        ...
        rate = rates.get(self.requested_vehicle_category, 7000)
        base = max(95000.0, dist * rate)
        return Decimal(str(int(base // 1000 * 1000)))

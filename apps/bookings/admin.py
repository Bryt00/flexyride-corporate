...
class TransportationRequestAdmin(ModelAdmin):
    ...
    def portal_direct_button(self, obj):
        if not obj.pk:
            return '-'
        broker_url = "/portal/broker/dispatches/"
        details_url = f"/portal/requests/{obj.request_number}/details/"
        return format_html(
            ...
            '<i data-lucide="shield" style="width:14px;height:14px;"></i> Open Broker Mission Control'
            '</a>'
            '<a href="{}" target="_blank" class="badge-action-link" style="display:inline-flex;padding:0.4rem 0.8rem;gap:0.4rem;opacity:0.85;">'
            '<i data-lucide="external-link" style="width:14px;height:14px;"></i> View Request Lifecycle'
            '</a>'
            '</div>',
            broker_url, details_url
        )

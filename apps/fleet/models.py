...
class VehiclePhoto(models.Model):
    ...
    def __str__(self):
        reg = getattr(self.vehicle, 'registration_number', 'N/A') if getattr(self, 'vehicle_id', None) else 'N/A'
        return f"Photo for {reg}"
...
class Driver(models.Model):
    ...
    def __str__(self):
        prov_name = getattr(self.provider, 'company_name', 'Independent') if getattr(self, 'provider_id', None) else 'Independent'
        return f"{self.full_name} ({prov_name}) [{self.get_status_display()}]"
